       ...
       if ( !empty( $_POST ) && wp_verify_nonce( $_POST['_wpnonce'], 'ssp-ajax-nonce' ) ) {
          ...
          <div class="ssp_divider" data-title="<?php the_title();?>" data-url="<?php the_permalink(); ?>" id="<?php echo get_the_ID();?>">
               ...
               if($markup   == "default_markup"){
                ...
                $orginalstr = array("#post_title", "#post_content",'#post_image', '#post_excerpt','#author_name','#postID','#post_day','#post_month','#post_year','#category_name','#post_class','#post_comment','#post_permalink','#post_target');
                $replacestr   = array($smart_post_titlee ,$smart_post_contents , $post_fimage , $post_excerpt, $author_name, $postID , $SmartDay, $SmartMonth,$Smartyear,$categorylists,$postclass_separated,$comment_number,$permalink,$post_link_target );
